...
export default function decorate(block) {
  ...
  const {
    yAxisUnit, yAxisPrefix, xAxisUnit, xAxisPrefix,
  } = chartConfig;

  // Apply optional axis units and prefixes if present
  if (yAxisUnit !== undefined || yAxisPrefix !== undefined) {
    chartConfig.yAxis = { ...chartConfig.yAxis };
    chartConfig.yAxis.labels = { ...chartConfig.yAxis.labels };
    chartConfig.yAxis.labels.format = `${yAxisPrefix || ''}{value}${yAxisUnit || ''}`;
  }

  if (xAxisUnit !== undefined || xAxisPrefix !== undefined) {
    chartConfig.xAxis = { ...chartConfig.xAxis };
    chartConfig.xAxis.labels = { ...chartConfig.xAxis.labels };
    chartConfig.xAxis.labels.formatter = function xAxisFormatter() {
      return `${xAxisPrefix || ''}${this.value}${xAxisUnit || ''}`;
    };
  }
  ...
  if (
    ...
    || xAxisUnit !== undefined
    || xAxisPrefix !== undefined) {
    const val = (v, def) => (v !== undefined && v !== null ? v : def);
    const yp = val(yAxisPrefix, '$');
    const yu = val(yAxisUnit, 'B');
    const xp = val(xAxisPrefix, '');
    const xu = val(xAxisUnit, '');

    chartConfig.tooltip = { ...chartConfig.tooltip };
    chartConfig.tooltip.formatter = function tooltipFormatter() {
      return `<div style="padding: 4px; text-align: center;">
                          <div style="font-size: 16px; font-weight: 600; color: #0273CF; margin-bottom: 4px;">${yp}${this.y}${yu}</div>
                          <div style="font-size: 12px; color: #6F7176;">${xp}${this.x}${xu} ${this.series.name}</div>
                        </div>`;
    };
  }
